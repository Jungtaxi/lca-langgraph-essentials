...
- 예시 형식:
  ...
  - "마음에 드는 곳이 없으면 '다 별로야', '그냥 알아서 골라줘'처럼 자유롭게 말씀하셔도 됩니다."
- 이 문장은 그대로 사용자에게 출력될 것입니다.
...
def agent4_suggest_node(state: AgentState) -> AgentState:
    ...
    # 디버그 정보 심어두고 싶으면 주석 해제
    # debug = state.get("debug", {})
    # debug["agent4_select_mode"] = mode
    # state["debug"] = debug
...
def agent4_wait_node(state: AgentState) -> AgentState:
    """
    agent4_suggest_node 이후 그래프를 잠깐 멈추는 '대기 노드'.
    (지금 구조에서는 그냥 state를 그대로 넘겨주기만 함)
    """
    return state
